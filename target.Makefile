...
SERVER=moto@fw0.magnum.com.ua
SRVDIR='~/www/$(TARGET)/lang/$(LANG)/amr/'

BUILD_PATH := ./build/$(TARGET)/$(LANG)
# FFMPEG_GAIN_vi_VN := -af "volume=3.0"
# FFMPEG_END_PAUSE_vi_VN := -i wav/end_pause.wav
FFMPEG_FLAGS := -acodec libopencore_amrnb -ab $(BITRATE) -ac 1 -ar 8k -y $(FFMPEG_GAIN_$(LANG))
...
$(BUILD_PATH)/amr/%.amr: wav/$(LANG)/%.wav
	@echo Convert $< to $@
	@$(MKDIR) $(dir $@)
	@$(FFMPEG) -i $< $(FFMPEG_FLAGS) $@ 2>/dev/null
