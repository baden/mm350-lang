...
#  1                MAX_PROG_SIZE
#  1                reserve
#  PROG_COUNT*MAX_PROG_SIZE   PROGRAMS: always MAX_PROG_SIZE steps
#  AMR_COUNT*(2+2)  AMR SIZE + CRC
...
for line in file(sys.argv[1]):
    ...
    max_progsize = max(max_progsize, len(amr_nums))
    for amr_num in amr_nums:
        max_amr = max(max_amr, int(amr_num, 10))
        amrs[int(amr_num, 10)] = True
...
wfile = open(sys.argv[2], 'wb')
wfile.write(struct.pack('<L', 0xBADE1977))
wfile.write(struct.pack('<B', max_program))
wfile.write(struct.pack('<B', max_amr))
wfile.write(struct.pack('<B', max_progsize))
wfile.write(struct.pack('<B', 0))
...
for p in range(0, max_program+1):
    ...
    # wline = "AMR(%03d):" % p
    for amr in program:
        wfile.write(struct.pack('<B', int(amr, 10)))
        # wline += " " + amr
        # try:
    ...
    for i in range(len(program), max_progsize):
        wfile.write(struct.pack('<B', 0xFF))
        # wline += " ---"
...
print "    AMRs = ", max_amr
# print "amrs = ", amrs
for amr in range(0, max_amr+1):
    if amr not in amrs:
        wfile.write(struct.pack('<H', 0xFFFF))
        wfile.write(struct.pack('<H', 0xFFFF))
    else:
        file = amr_dir + ("/%03d.amr" % amr)
        if not os.path.isfile(file):
            print "  Warning!: AMR-file %s is not exist " % file
        ...
        wfile.write(struct.pack('<H', size))
        wfile.write(struct.pack('<H', crc))
